    ...
    <div class="content-wrapper">
        <div class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1 class="m-0">Grupos de cursos</h1>
                    </div>
                </div>
            </div>
        </div>
                    ...
                    <div class="col-lg-12">
                        <div class="card card-primary card-outline">
                            <div class="card-header">
                                <h5 class="m-0">Búsqueda de grupos de cursos</h5>
                            </div>
                            <div class="card-body">
...
@section('javascript')
    <script>
        ...
        function modalEditar(id) {
            const ruta = route('grupo.edit', [id]);
            axios.get(ruta)
                .then(function(respuesta) {
        ...
        function actualizar(id) {
            const ruta = route('grupo.update', [id]);
        ...
        function confirmarEliminar(id) {
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    const ruta = route('grupo.destroy', [id]);
        ...
        function cargarCursos() {
            const tipo_curso_id = $('#tipo_curso_id').val();
            if (tipo_curso_id === '') {
                $('#curso_id').html(`<option value="">[--SELECCIONE--]</option>`)
                return
            }
            console.log(tipo_curso_id)
            axios.get(route('curso.cargar_por_tipo', [tipo_curso_id]))
                .then(function(respuesta) {
                    $('#curso_id').html(respuesta.data)
                })
                .catch(function() {
                    toastr('Error al cargar cursos')
                    $('#curso_id').html(`<option value="">[--SELECCIONE--]</option>`)
                });
        }
    </script>
